...
@time: 2019/10/14 15:42
@desc:
'''
import re


class ProtocolCheck(object):
    """
        校验各类协议类
    """
    ...
    def check_gb(self, data):
        ...
        """
        try:
            data_b = bytes().fromhex(data)  # 把十六进制数字字符串转换为一个字节对象，并接受两个数字间的空格
            n = len(data_b)
            b = data_b[1]
    ...
    def check_v3(line):
        ...
        :return:
        """
        crc_16tab = [0x0000, 0x1189, 0x2312, 0x329B, 0x4624, 0x57AD, 0x6536, 0x74BF, 0x8C48, 0x9DC1, 0xAF5A,
        ...
        # 判断是否是十六进制字符串
        def isHexData(string):
            reg = "^[0-9A-Fa-f\\s]+$"
            return re.search(reg, string)

        def GetCrc16(data, offset, ln):
            preval = 0xffff
            for i in range(ln):
                preval = (preval >> 8) ^ crc_16tab[(preval ^ data[i + offset]) & 0xff]
            return ~preval & 0xFFFF

        try:
            if line == "" or line is None:
                print("校验数据不能为空！")
            elif not (isHexData(line)):
                print("数据错误[非十六进制数据]！")
            else:
                da = [x for x in bytes().fromhex(line)]
                lenn = len(da)
                if lenn < 15:
                    print("数据长度太短[最小15字节]！")
                else:
                    xor = GetCrc16(da, 2, lenn - 6)
                    da[lenn - 4] = (xor & 0xff00) >> 8
                    da[lenn - 3] = xor & 0x00ff
                    st = ""
                    for j in da:
                        ss = str(hex(j))[2:].zfill(2)
                        st = st + ss
                    st = st.upper()
                    return st
        except Exception as e:
            print("校验传入的V3协议数据失败:%s" % e)
...
if __name__ == '__main__':
    a = ProtocolCheck()
    # bsj='292980002880008181190213120200022320041140156000000000F80138807EFC0000001E000000000000110D'
    # s=a.check_bsj(bsj)
    # print(s)
    data='23 23 02 35 35 36 36 37 37 35 35 36 36 37 37 35 35 36 36 37 02 01 00 a1 13 0b 04 0b 1d 20 03 C8 01 00 00 00 00 00 00 35 35 36 36 37 37 35 35 36 36 37 37 35 35 36 36 37 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 00 02 32 00 64 af af 01 90 03 e8 13 88 13 88 7d 03 e8 28 60 28 60 01 f4 5a 7d 01 07 3b 5e 64 01 d4 a9 da 00 00 03 e8 80 01 7d 00 00 00 64 5a 00 00 13 88 00 00 00 32 28 60'
    s = a.check_gb(data)
    print(s)
    # data = ''
    # a = get_xor(data)
    # b = get_bcc(a)
    # print(b)
